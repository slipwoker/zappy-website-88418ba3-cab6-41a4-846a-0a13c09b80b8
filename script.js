  ...
  if (contactForm) {
    ...
    function getFieldError(field) {
      ...
      if (value.trim() !== '') {
        ...
        if ((type === 'tel' || name === 'phone') && !VALIDATION_RULES.phone(value)) {
          return field.dataset.errorPhone || 'Please enter a valid phone number.';
        }

        if (minLength && !VALIDATION_RULES.minLength(value, minLength)) {
          return field.dataset.errorMinlength || 'Minimum length is ' + minLength + ' characters.';
        }
      }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      const successMessage = contactForm.dataset.successMessage || 'Thank you! Your message has been sent successfully.';
      const errorMessage = contactForm.dataset.errorMessage || 'Something went wrong. Please try again.';
      ...
      fetch(action, {
        ...
        .then(function (response) {
          if (response.ok) {
            showFormMessage(contactForm, successMessage, 'success');
            contactForm.reset();
            fields.forEach(function (field) {
              clearFieldError(field);
            });
        ...
        .catch(function (err) {
          showFormMessage(contactForm, err.message || errorMessage, 'error');
        })
        .finally(function () {
          if (submitBtn) {
    ...
    function showFormMessage(form, message, type) {
      const existing = form.querySelector('.form-status-message');
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "המתכונים הכי ישראלים משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '88418ba3-cab6-41a4-846a-0a13c09b80b8',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '88418ba3-cab6-41a4-846a-0a13c09b80b8';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '88418ba3-cab6-41a4-846a-0a13c09b80b8';
